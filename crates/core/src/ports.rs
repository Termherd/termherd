...
pub trait PtyHost: Send + Sync {
    ...
    /// decides, not the caller: only it holds the live mode that says whether
    /// the child reads the mouse — forwarded as a report — or the terminal's
    /// own selection moves, and the live scroll offset that places it.
